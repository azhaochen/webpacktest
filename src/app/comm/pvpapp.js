;define(function(require, exports,module){
	...
	*/
	function openURL(method, params){
		var url = 'igameapp://jsapi/'+ method + '?p=' + encodeURIComponent(JSON.stringify(params) || '');
	...
	function openApp(url,download_adtag,directdownload){
		...
		if (u.indexOf('Android') > -1 || u.indexOf('Linux') > -1) ////android终端或者uc浏览器
		{
			if (url.indexOf("http://") > -1 || url.indexOf("https://") > -1 ) {
				g_sSchema   = 'tencent://igame.app/open?url='+encodeURIComponent(url)+'&v='+(new Date()).getTime();   //拉起H5地址
			}else{
				g_sSchema   = 'tencent://igame.app/open?'+url+'&v='+(new Date()).getTime();   //拉起native地址
			}
		}
		else //ios也发送
		{
	    ...
	    //下载页改为APP官网首页
		g_sDownload = 'http://igame.qq.com/pvp/download/index.php?_t=1';
		if(download_adtag){
	...
	*/
	function openAppNotDownload(url,download_adtag){
		...
		//下载页改为APP官网首页
		var g_sDownload = '//igame.qq.com/pvp/download/index.php';
		if(download_adtag){
			g_sDownload += '?adtag='+download_adtag;
		}
		//创建iframe，呼起app schema
